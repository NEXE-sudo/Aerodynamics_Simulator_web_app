...
export default function ComparisonPanel({
  ...
  const renderMetricComparison = (label: string, currentVal: number, savedVal: number, unit: string = '') => {
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-xs font-bold ${isImprovement ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <div className="text-lg font-bold text-teal-600">
              {currentVal.toFixed(3)}{unit}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Baseline</div>
            <div className="text-lg font-bold text-gray-700">
              {savedVal.toFixed(3)}{unit}
            </div>
          </div>
        </div>
        {diff !== 0 && (
          <div className={`text-xs mt-2 pt-2 border-t border-gray-300 ${getChangeColor(percentChange)}`}>
            Δ = {diff > 0 ? '+' : ''}{diff.toFixed(3)}{unit}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="text-xs font-medium text-gray-700 mb-2">Flow Regime</div>
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span className="text-gray-600">Current:</span>
            <div className="font-semibold text-blue-700">{currentResults.regime.type}</div>
          </div>
          <div>
            <span className="text-gray-600">Baseline:</span>
            <div className="font-semibold text-gray-700">{savedResults.regime.type}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1">
        {renderMetricComparison('Lift Coefficient (CL)', currentResults.cl.nominal, savedResults.cl.nominal)}
        {renderMetricComparison('Drag Coefficient (CD)', currentResults.cd.nominal, savedResults.cd.nominal)}
        {renderMetricComparison('Lift Force', currentResults.lift.nominal, savedResults.lift.nominal, ' N')}
        {renderMetricComparison('Drag Force', currentResults.drag.nominal, savedResults.drag.nominal, ' N')}
        {renderMetricComparison('L/D Ratio', currentResults.efficiency.nominal, savedResults.efficiency.nominal)}
      </div>

      {/* Reynolds and Stability Comparison */}
      <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-200">
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
          <div className="text-xs text-gray-600 mb-1 font-medium">Reynolds Number</div>
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-3">
          <div className="text-xs text-gray-600 mb-1 font-medium">Stability</div>
